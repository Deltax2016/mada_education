...
def cover(seed: str) -> str:
    """Drawn art committed with the site, not a stock photograph by seed.

    Random photography gave a tax course a beach. These are generated by
    scripts/make_covers.py and served from the web app.
    """
    return f"/covers/{seed}.svg"


def avatar(seed: str) -> str:
    return f"/avatars/{seed}.svg"
...
async def refresh_placeholder_art(db: AsyncSession) -> int:
    """Replace stock photo placeholders with the drawn art.

    Deployments seeded before the art existed still hold picsum URLs, and the
    catalogue loader deliberately never updates anything. Only URLs pointing at
    the placeholder service are touched, so a cover an author uploaded is left
    exactly as it is.
    """
    changed = 0

    courses = (
        await db.execute(select(Course).where(Course.cover_url.like("%picsum.photos%")))
    ).scalars()
    by_cover = {slug: meta["cover"] for slug, meta in META.items()}
    for course in courses:
        name = by_cover.get(course.slug)
        if name:
            course.cover_url = cover(name)
            changed += 1

    users = (
        await db.execute(select(User).where(User.avatar_url.like("%picsum.photos%")))
    ).scalars()
    by_avatar = {i["email"]: i["avatar"] for i in INSTRUCTORS}
    for user in users:
        name = by_avatar.get(user.email)
        if name:
            user.avatar_url = avatar(name)
            changed += 1
